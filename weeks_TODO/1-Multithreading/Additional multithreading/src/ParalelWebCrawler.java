...
public class ParalelWebCrawler {
	...
	public ParalelWebCrawler(ListeningExecutorService service) {
		this.service = service;
	}

	public Set<URI> crawl(URI currentLocation, final String needle, AtomicInteger parentInt) throws URISyntaxException, 
			MalformedURLException, InterruptedException, ExecutionException { 
		Set<URI> finalResult = new ConcurrentSkipListSet<URI>();
		...
		}
		{
			...
			for (URI link : filterURIs) {
				if (!visitedUrls.contains(link)) { // visitedUrls.add(currentLocation);
					i.incrementAndGet();
					ListenableFuture<Set<URI>> a = service.submit(new Callable<Set<URI>>() {
						@Override
					...
					Futures.addCallback(a, new FutureCallback<Set<URI>>() {
						@Override
			...
			for (ListenableFuture<Set<URI>> resultListenableFuture : futures) {
				Set<URI> result = resultListenableFuture.get();
				if (!result.isEmpty()) {
					finalResult.addAll(result);
				}
			}
